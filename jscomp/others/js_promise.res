...
@val @scope("Promise") external all: array<t<'a>> => t<array<'a>> = "all"
@val @scope("Promise") external all2: ((t<'a0>, t<'a1>)) => t<('a0, 'a1)> = "all"
@val @scope("Promise") external all3: ((t<'a0>, t<'a1>, t<'a2>)) => t<('a0, 'a1, 'a2)> = "all"
@val @scope("Promise")
...
@val @scope("Promise") external race: array<t<'a>> => t<'a> = "race"

@bs.send.pipe(: t<'a>) external then_: (@uncurry ('a => t<'b>)) => t<'b> = "then"

@bs.send.pipe(: t<'a>) external catch: (@uncurry (error => t<'a>)) => t<'a> = "catch"
/* ` p|> catch handler`
